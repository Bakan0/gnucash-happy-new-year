...
OPENING_DATE = (1, 1, 2024)  # day, month, year
...
balance_accounts: dict, optional
    ...
    # Create target if it does not exist yet, or use existing one
    if not os.path.exists(target):
        target_session_mode = SessionOpenMode.SESSION_NEW_STORE
    else:
        print(f"Warning! File {target} exists already.")
        target_session_mode = SessionOpenMode.SESSION_NORMAL_OPEN

    with (Session(old, SessionOpenMode.SESSION_READ_ONLY) as original_book_session,
          Session(target_sqlite, target_session_mode) as target_book_session):
